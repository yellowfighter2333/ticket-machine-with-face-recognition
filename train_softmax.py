...
def parse_arguments(argv):#通过该argument来获取获取参数
    parser = argparse.ArgumentParser()
    
    parser.add_argument('--logs_base_dir', type=str, 
        help='Directory where to write event logs.', default='~/logs/facenet')#日志的位置
    parser.add_argument('--models_base_dir', type=str,
        help='Directory where to write trained models and checkpoints.', default='~/models/facenet')
    parser.add_argument('--gpu_memory_fraction', type=float,
        help='Upper bound on the amount of GPU memory that will be used by the process.', default=0.6)#初始设定为1.0，但是容易出现gpu内存填满的情况，因此使用较小的比例0.6
    parser.add_argument('--pretrained_model', type=str,
        help='Load a pretrained model before training starts.')
    parser.add_argument('--data_dir', type=str,
        help='Path to the data directory containing aligned face patches.',
        default='~/datasets/casia/casia_maxpy_mtcnnalign_182_160')#dataset的地址，这个里面使用的dataset为casia数据集
    parser.add_argument('--model_def', type=str,
        help='Model definition. Points to a module containing the definition of the inference graph.', default='models.inception_resnet_v1')#具体的到时哪个model，默认为inception_resnet_v1
    ...
    parser.add_argument('--lfw_file_ext', type=str,
        help='The file extension for the LFW dataset.', default='png', choices=['jpg', 'png'])#lfw数据集中的拓展名
...
if __name__ == '__main__':
    main(parse_arguments(sys.argv[1:])#运行Main函数以，argv[1:]作为输入
